...
class DBOperations {
  ...
  public function getContactsByUser($user_id){
    $com = new DbConnect();
    $sql = "SELECT user_id, name, email, image, last_seen, status from friends, users WHERE ( ( '$user_id' != users.user_id and '$user_id' = friends.request_user_id  and users.user_id = friends.approved_user_id  ) or ('$user_id' != users.user_id and '$user_id'= friends.approved_user_id and users.user_id = friends.request_user_id and type = '1' ) ) ";
